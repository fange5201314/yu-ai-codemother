...
public class AppServiceImpl extends ServiceImpl<AppMapper, App>  implements AppService{
    ...
    public Flux<String> chatToGenCode(Long appId, String message, User loginUser) {
        ...
        // 5. 通过校验之后，添加用户消息到对话历史中
        chatHistoryService.addChatMessage(appId, message, ChatHistoryMessageTypeEnum.USER.getValue(), loginUser.getId());
        // 6. 调用 AI 生成代码
        Flux<String> contentFlux = aiCodeGeneratorFacade.generateAndSaveCodeStream(message, codeGenTypeEnum, appId);
        // 7. 将 AI 生成代码的响应流转换为字符串返回前端成功之后将ai响应对话放置到对话历史中
        ...
        return contentFlux.map(chunk ->{
            ...
            //将ai响应的消息放置到对话历史中
            String responseMessage = aiResponseBuilder.toString();
            chatHistoryService.addChatMessage(appId, responseMessage, ChatHistoryMessageTypeEnum.AI.getValue(), loginUser.getId());
        }).doOnError(error->{
